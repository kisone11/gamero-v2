...
import { z } from 'zod'
import { Plus, X, Upload, ArrowLeft, Zap } from 'lucide-react'
...
export default function EditProjectPage() {
  ...
  useEffect(() => {
    ...
    setStyleTags(project.style_tags || [])
    setPlatforms(project.platform || [])
    setFormInitialized(true)
  }, [project, formInitialized, reset])

  useEffect(() => {
    if (formInitialized && (isDirty || !!coverFile || screenshotFiles.length > 0 || !!videoFile)) {
      setDirty(true)
    }
  }, [formInitialized, isDirty, coverFile, screenshotFiles, videoFile])
  ...
  const addTag = () => {
    ...
    if (styleTags.includes(tag)) return
  ...
  const handleDeleteVideo = async () => {
    if (!project) return
    const videoKey = project.video_keys?.[0]
    if (!videoKey) return
    try {
      await projectApi.deleteVideo(project.id, videoKey)
      toast.success('视频已删除')
      queryClient.invalidateQueries({ queryKey: ['project', slug] })
    } catch {
      toast.error('删除视频失败')
  ...
  const handleDeleteScreenshot = async (key: string) => {
    if (!project) return
    try {
      await projectApi.deleteScreenshot(project.id, key)
      toast.success('截图已删除')
      queryClient.invalidateQueries({ queryKey: ['project', slug] })
    } catch {
      toast.error('删除截图失败')
  ...
  if (projectQuery.isLoading) {
    return (
      <div className="max-w-3xl mx-auto px-6 py-8">
        <div className="h-10 w-48 bg-white/[0.04] rounded animate-pulse mb-8" />
    ...
    return (
      <div className="max-w-3xl mx-auto px-6 py-8 min-h-[60vh] flex items-center justify-center">
        <div className="flex flex-col items-center justify-center text-center">
          ...
          <p className="text-[13px] text-text-muted mb-6">无法加载项目信息，请检查后重试</p>
          <Button variant="secondary" size="sm" onClick={() => projectQuery.refetch()}>重新加载</Button>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="max-w-3xl mx-auto px-6 py-8">
      <Link to={`/p/${slug}`} className="inline-flex items-center gap-1.5 text-[13px] text-text-muted hover:text-amber transition-colors mb-6">
        <ArrowLeft className="w-4 h-4" /> 返回项目
      </Link>
      <h1 className="text-[22px] font-bold text-text-primary mb-8">编辑项目 · {project.name}</h1>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div className="bg-surface-card border border-white/[0.04] rounded-xl p-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="flex flex-col gap-1.5">
              <label className="text-[12px] font-semibold text-text-secondary uppercase tracking-wider">项目类型</label>
              <select
              ...
              {errors.genre && <p className="text-[13px] text-danger">{errors.genre.message}</p>}
            </div>
            <div className="flex flex-col gap-1.5">
              <label className="text-[12px] font-semibold text-text-secondary uppercase tracking-wider">项目状态</label>
              <select
                className="h-10 px-3 bg-surface-void border border-white/[0.04] rounded-xl text-[14px] text-text-primary focus:outline-none focus:border-amber"
                {...register('status')}
              >
        ...
        <div className="bg-surface-card border border-white/[0.04] rounded-xl p-5">
          ...
          <div className="flex gap-2 mb-3">
            <input
              className="flex-1 h-10 px-3 bg-surface-void border border-white/[0.04] rounded-xl text-[14px] text-text-primary placeholder:text-text-muted focus:outline-none focus:border-amber"
        ...
        <div className="bg-surface-card border border-white/[0.04] rounded-xl p-5">
          ...
          <div className="space-y-4">
            <div className="flex flex-col gap-1.5">
              <label className="text-[12px] font-semibold text-text-secondary uppercase tracking-wider">目标平台</label>
              <div className="flex flex-wrap gap-2">
            ...
            <div className="flex flex-col gap-1.5">
              <label className="text-[12px] font-semibold text-text-secondary uppercase tracking-wider">开发引擎</label>
              <select
                className="h-10 px-3 bg-surface-void border border-white/[0.04] rounded-xl text-[14px] text-text-primary focus:outline-none focus:border-amber"
                {...register('engine')}
              >
        ...
        <div className="bg-surface-card border border-white/[0.04] rounded-xl p-5">
          ...
          {!coverFile && project.cover_url && (
            <div className="relative w-full h-48 rounded-xl overflow-hidden border border-white/[0.04] mb-3">
              <img src={project.cover_url} alt="" className="w-full h-full object-cover" />
              <span className="absolute top-2 right-2 px-2 py-0.5 bg-black/50 text-white text-[11px] rounded-full">当前封面</span>
            </div>
          )}
          {coverPreview && (
            <div className="relative w-full h-48 rounded-xl overflow-hidden border border-white/[0.04] mb-3">
              <img src={coverPreview} alt="" className="w-full h-full object-cover" />
              <button type="button" onClick={removeCover} className="absolute top-2 right-2 p-1 bg-black/50 rounded-full text-white hover:bg-black/70">
                <X className="h-4 w-4" />
              </button>
            </div>
          )}
          ...
          >
            ...
            <span className="text-[13px] text-text-muted font-semibold">
              {coverFile || project.cover_url ? '更换封面' : '上传封面'}
            </span>
          </button>
        </div>
            ...
            <div className="grid grid-cols-3 gap-3 mb-4">
              {project.screenshot_urls.map((url, i) => (
                <div key={i} className="relative aspect-video rounded-xl overflow-hidden border border-white/[0.04] group">
                  ...
                  <button
                    type="button"
                    onClick={() => handleDeleteScreenshot(project.screenshot_keys?.[i] || '')}
          ...
          {screenshotPreviews.length > 0 && (
            <div className="grid grid-cols-3 gap-3 mb-4">
              {screenshotPreviews.map((preview, i) => (
                <div key={`new-${i}`} className="relative aspect-video rounded-xl overflow-hidden border border-white/[0.04]">
                  <img src={preview} alt="" className="w-full h-full object-cover" />
                  <button type="button" onClick={() => removeNewScreenshot(i)} className="absolute top-1 right-1 p-0.5 bg-black/50 rounded-full text-white hover:bg-black/70">
                    <X className="h-3 w-3" />
                  </button>
                </div>
              ))}
        ...
        <div className="bg-surface-card border border-white/[0.04] rounded-xl p-5">
          ...
          {(project.video_url || (project.video_urls?.length ?? 0) > 0) && (
            <div className="mb-3">
              <label className="text-[12px] font-semibold text-text-secondary uppercase tracking-wider block mb-2">当前宣传视频</label>
              <div className="relative rounded-xl overflow-hidden border border-white/[0.04] max-h-48 group">
                <video src={project.video_url || project.video_urls![0]} controls className="w-full" />
                <button type="button" onClick={handleDeleteVideo} className="absolute top-2 right-2 p-1 bg-black/50 rounded-full text-white hover:bg-black/70 opacity-0 group-hover:opacity-100 transition-opacity">
                  <X className="h-4 w-4" />
                </button>
              </div>
            </div>
          )}
          <input ref={videoInputRef} type="file" accept="video/mp4,video/quicktime,video/webm" onChange={handleVideoChange} className="hidden" />
          {videoPreview ? (
            <div className="relative w-full rounded-xl overflow-hidden border border-white/[0.04] mb-3">
        ...
        <div className="bg-surface-card border border-white/[0.04] rounded-xl p-5">
          ...
          <div className="space-y-4">
            <Input label="试玩链接" placeholder="https://..." {...register('demo_url')} fullWidth />
            <Input label="商店链接" placeholder="https://..." {...register('store_url')} fullWidth />
          </div>
        </div>

        <div className="flex items-center gap-3 justify-end">
          <Button type="button" variant="ghost" onClick={() => navigate(`/p/${slug}`)}>取消</Button>
          <Button type="submit" loading={updateMutation.isPending}>保存更改</Button>
        </div>
      </form>
    </div>
  )
